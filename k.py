...
# Преобразование из шестнадцатеричного в целое число
def hex_to_int(hex_string):
    return int(hex_string, 16)

# Преобразование из целого числа в шестнадцатеричный формат
def int_to_hex(integer):
    return hex(integer)[2:]

# Преобразование из русского слова в целое число
def russian_word_to_int(russian_word):
    return int(russian_word.encode('utf-8').hex(), 16)

# Преобразование из целого числа в русское слово
def int_to_russian_word(integer):
    return bytes.fromhex(hex(integer)[2:]).decode('latin-1', errors='ignore')


# Функция S: Ввод x и вывод y имеют размер 128 бит
def S(x):
    y = 0
...
# Функция S_inv: Ввод x и вывод y имеют размер 128 бит
def S_inv(x):
    y = 0
    for i in range(16):
        y <<= 8
...
# x и y - неотрицательные целые числа
# Их связанные бинарные полиномы умножаются.
# Возвращается соответствующее целое число этому произведению.
...
# Возвращает количество бит, используемых для хранения положительного целого числа x.
def number_bits(x):
    nb = 0
    while x != 0:
        nb += 1
...
# x и y - 8 бит
# Выходное значение - 8 бит
def kuznyechik_multiplication(x, y):
    z = multiply_ints_as_polynomials(x, y)
    m = int('111000011', 2)
    return mod_int_as_polynomial(z, m)

# Функция линейного функционала: Вход x - 128 бит (рассматривается как вектор из шестнадцати байтов)
# Возвращается 8 бит
...
lines, PT, k = read_input(input_file)
...
# Преобразование расшифрованного текста в русское слово
decrypted_text = int_to_russian_word(DT)

# Проверка расшифрованного текста с оригинальным
result = "Расшифрованный текст совпадает с оригинальным: {}".format(decrypted_text == lines[0].strip())
